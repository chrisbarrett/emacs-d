;;; ui-tabs.el --- Tab management with transient menu -*- lexical-binding: t; -*-
...
(with-eval-after-load 'transient
  (transient-define-prefix +tabs-menu--impl ()
...
(defun +tab-bar-set-transient-alert--impl (&optional tab-name color cycles)
  "Implementation for `+tab-bar-set-transient-alert'."
  (let* ((tabs (frame-parameter nil 'tabs))
...
(defun +tab-bar-set-alert--impl (&optional tab-name)
  "Implementation for `+tab-bar-set-alert'."
  (let* ((tabs (frame-parameter nil 'tabs))
         (tab-index (if tab-name
                        (seq-position tabs tab-name
                                      (lambda (tab name)
...
(defun +tab-bar-clear-alert--impl (&optional tab-name)
  "Implementation for `+tab-bar-clear-alert'."
  (let* ((tabs (frame-parameter nil 'tabs))
         (tab-index (if tab-name
                        (seq-position tabs tab-name
                                      (lambda (tab name)
...
(defun +update-tab-bar-themes--impl (&rest _)
