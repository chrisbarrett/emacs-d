...
- [ ] 1.1 Create `modules/presentation/` with `init.el`, `lib.el`,
  `tests.el`, `packages.eld`, `spec.md` headers and `provide` forms
- [ ] 1.2 Register the module in the main module loader
- [ ] 1.3 Add a `defgroup +presentation` and the `+presentation--sessions`
  hash defvar

## 2. Effect data types (TDD)

- [ ] 2.1 Add failing test that the planner returns a list of
  `+presentation-effect-shell` and `+presentation-effect-elisp` records
  for a representative spawn-path input
- [ ] 2.2 Define `cl-defstruct +presentation-effect-shell` (with `argv`
  field) and `+presentation-effect-elisp` (with `thunk` field)
- [ ] 2.3 Add failing test that `+presentation--run-effects` shells out
  ...
  in the test
- [ ] 2.4 Implement `+presentation--run-effects`

## 3. Tmux command builders (TDD, pure)

- [ ] 3.1 Add failing test that `+presentation--cmd-list-panes` emits the
  exact `tmux list-panes -t SESS:WIN -F #{pane_id}\t#{pane_tty}` argv
- [ ] 3.2 Implement the builder
- [ ] 3.3 Add failing tests for `+presentation--cmd-split-window`
  covering both horizontal and vertical splits and the
  `emacsclient -t -s SOCKET` tail
- [ ] 3.4 Implement the builder
- [ ] 3.5 Add failing test for `+presentation--cmd-kill-pane`
- [ ] 3.6 Implement the builder
- [ ] 3.7 Add failing test for `+presentation--parse-list-panes-output`
  given a multi-line `pane_id\tpane_tty` string; assert it returns
  an alist `((pane-id . tty) ...)`
- [ ] 3.8 Implement the parser

## 4. State store + key generation

- [ ] 4.1 Add failing test that `+presentation--make-key` returns a
  fresh, unique string each call
- [ ] 4.2 Implement using `gensym` or equivalent
- [ ] 4.3 Add failing test that `+presentation--register-session`
  inserts a plist into the hash and tags the frame with
  `presentation-key` and `presentation-origin`
- [ ] 4.4 Implement
- [ ] 4.5 Add failing test that `+presentation--get-session` returns
  the plist for a valid key and signals a user-error for an unknown key
- [ ] 4.6 Implement

## 5. Frame discovery (TDD)

- [ ] 5.1 Add failing test that `+presentation--find-frame-by-tty`
  given a tty string returns the matching frame from a faked
  `frame-list`/`frame-parameter` environment
- [ ] 5.2 Implement
- [ ] 5.3 Add failing test that `+presentation--find-existing-frame`
...
- [ ] 6.1 Add failing test that `+presentation--make-splash-buffer`
...
- [ ] 8.2 Implement; ensure the plan also emits an effect to push the
  config to register `?P`

## 9. Created-frame path orchestration (TDD)

- [ ] 9.1 Add failing test that `+presentation--plan-spawn` emits, in
...
- [ ] 9.2 Implement
- [ ] 9.3 Add failing test that the post-split pane discovery picks
  the pane that's in `after` but not in `before`
- [ ] 9.4 Implement `+presentation--diff-panes`

## 10. start_presentation entry point (TDD)

- [ ] 10.1 Add failing test that `+presentation-start` with a payload
  whose tmux discovery yields an existing frame takes the reuse path
  and returns a session key
- [ ] 10.2 Add failing test that with no existing frame it takes the
  spawn path and returns a session key
- [ ] 10.3 Add failing test that an `initial_slide` in the payload is
...
- [ ] 10.4 Implement `+presentation-start`

## 11. end_presentation entry point (TDD)

- [ ] 11.1 Add failing test that `+presentation-end` on a `'reused`
  session emits the `set-window-configuration` effect, drops frame
  params, and removes the hash entry
- [ ] 11.2 Add failing test that `+presentation-end` on a `'created`
  session emits a `kill-pane` effect for the recorded pane id
- [ ] 11.3 Add failing test that `+presentation-end` on an unknown key
  signals a user-error
- [ ] 11.4 Implement

## 12. Frame-deletion cleanup hook

- [ ] 12.1 Add failing test that `+presentation--frame-deleted-h`
  removes the matching hash entry when called with a frame carrying
  a known `presentation-key`
- [ ] 12.2 Add failing test that the hook is a no-op for frames with
  no `presentation-key` parameter
- [ ] 12.3 Implement and wire into `delete-frame-functions` from
  `init.el`

## 13. display-buffer protection

- [ ] 13.1 Add failing test that with a faked
