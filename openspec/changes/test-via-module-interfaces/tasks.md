...
- [ ] 1.1 `modules/evil/tests.el`: replace each source-substring test
...
- [ ] 1.2 `modules/org-capture/tests.el`: same treatment for its
      `insert-file-contents` init.el assertions
- [ ] 1.3 `modules/vulpea/tests.el`: same treatment

## 2. Audit remaining search-forward suites

- [ ] 2.1 Audit `search-forward` usage in lang-elixir, lang-markdown,
...
- [ ] 2.2 Rewrite any module-source searches found; drop assertions
      with no observable consequence, noting each in the commit message

## 3. Close out

- [ ] 3.1 Verify the spec scenario: no `insert-file-contents` of a
      module's own init.el/lib.el remains under `modules/*/tests.el`
- [ ] 3.2 Full `make test` green; spot-check that a whitespace-only
