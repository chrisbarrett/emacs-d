...
**Reason**: All MCP tool registration is removed; the public elisp
function `+present-markdown` replaces this surface.

**Migration**: Agents invoke `emacsclient -t -e '(+present-markdown
PATH)'` from a tmux pane (or similar).  No session key is involved;
the doc buffer is the session.  `end_presentation` is replaced by
killing the doc buffer or invoking `+presentation-quit` (`C-c q`).
`get_presentation` has no replacement; if the agent needs to inspect
state, it asks the user.

### Requirement: Frame reuse via tty match

**Reason**: Frame management is delegated to emacsclient and to
caller-side tmux invocations.  The presentation module no longer
discovers, spawns, or owns frames.

**Migration**: Callers control frame placement directly via the
`emacsclient` invocation.  Use `emacsclient -t` for terminal-mode
frames in the current tmux pane; use `emacsclient -c` for graphical
frames.

### Requirement: Effect-interpreter for tmux interaction

**Reason**: No tmux interaction remains in the elisp code.

**Migration**: None.  Callers manage tmux geometry themselves.

### Requirement: Session state book-keeping

**Reason**: The `+presentation--sessions` hash is removed.  All
state lives buffer-local on the doc buffer.

**Migration**: None.  Callers do not need a session key.

### Requirement: Tear-down by origin

**Reason**: There is no session lifecycle owned by the module; no
tear-down to perform.

**Migration**: Killing the doc buffer or invoking
`+presentation-quit` (`C-c q`) widens the buffer and (for buffers
opened solely by `+present-markdown`) kills it.  Tmux pane teardown
is the caller's problem if it spawned a pane.

### Requirement: Mid-session frame deletion cleanup

**Reason**: No session state to clean up.

**Migration**: None.

### Requirement: Splash buffer

**Reason**: No deck-with-no-current-slide state exists; the doc
buffer is itself what is shown.

**Migration**: None.

### Requirement: Narrative slide rendering

**Reason**: Slide kinds are removed; the doc buffer is rendered by
markdown-mode (or a derived mode) directly.

**Migration**: None.  All narrative content is the markdown body.

### Requirement: Deck mutation MCP tools

**Reason**: No deck data structure exists; mutation is "edit the
markdown file" via the agent's `Edit` / `Write` tools.

**Migration**: Agents edit the doc on disk; `+presentation-mode`'s
revert hooks re-narrow when the buffer auto-reverts.

### Requirement: push_slide tool semantics

**Reason**: `push_slide` is removed.

**Migration**: None.

### Requirement: Deck recovery via get_deck

**Reason**: `get_deck` is removed.

**Migration**: The agent reads the markdown file directly if it
needs to know the structure.

### Requirement: file slide kind

**Reason**: File slides are removed as a kind.  Source content is
embedded inline in the doc as `path#L<a>-L<b>` link previews; the
narrowed-buffer rendering survives as the click action for those
links (see `Reusable narrowed-source renderer`).

**Migration**: Replace `kind: "file"` slides with markdown links of
the form `[label](path#Lx-Ly)` in the doc body.

### Requirement: diff slide kind

**Reason**: Diff slides are removed as a kind.  Diffs are embedded
inline as `diff:<base>...<head>[#path]` link previews; clicking
opens `magit-diff-range`.

**Migration**: Replace `kind: "diff"` slides with markdown links of
the form `[label](diff:base...head)` in the doc body.

### Requirement: layout slide kind

**Reason**: Layout slides are removed.  Side-by-side viewing of two
files is achieved by clicking source links and using
`other-window-prefix` to control split direction.

**Migration**: There is no direct replacement for an agent-composed
two-pane layout.  Agents author docs that link to the two files;
the user splits as they read.

### Requirement: Per-slide annotation overlays

**Reason**: Annotations are no longer authored as slide payload
data.  The doc body uses prose / GFM callouts / inline comments to
convey what annotations did.

**Migration**: For per-line emphasis on source previews, click
through to the real buffer (which still applies a focus overlay) or
add commentary as prose adjacent to the link.

### Requirement: Slide validation

**Reason**: There are no slide objects to validate.

**Migration**: None.

### Requirement: Slide pane_layout hint

**Reason**: Per-slide tmux reshape is removed; tmux geometry is the
caller's concern.

**Migration**: Set tmux geometry via `tmux select-layout` before
invoking emacsclient.

### Requirement: Tmux window-layout save and restore around session lifetime

**Reason**: No session lifetime; no tmux interaction.

**Migration**: Same as above.

### Requirement: User-driven navigation emits a channel notification

**Reason**: Channel back-signal is removed.  Not supported on the
user's current Claude Code build, and not needed for the doc-first
flow.

**Migration**: The agent does not learn the user's slide position.
If correlation is needed, the agent asks the user.

### Requirement: Channel capability declared by MCP server

**Reason**: No channel notifications are emitted.

**Migration**: None.

### Requirement: Annotation kinds

**Reason**: Annotations are removed.
