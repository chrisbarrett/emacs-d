...
- [x] 1.1 Add a `gfm-pretty-tests` ert test that enables `gfm-pretty-mode`
...
- [x] 1.2 Add a second ert test, tagged `narrowing-regression`, that
...
- [x] 2.1 In `lisp/gfm/gfm-pretty-tables.el`, inside
...
- [x] 2.2 Confirm both new ert tests now pass.
...
- [x] 3.1 Open
      ...
      pipes, no `│ │ │ │ │ ` prefix).
- [x] 3.2 Page back and forth across the slide several times to confirm
      repeated `--reconcile-windows` invocations do not corrupt the
      overlay set.

## 4. Test suite

- [x] 4.1 Run `make test-quick` and confirm pass.
- [x] 4.2 Run `make test` and confirm pass (covers the
