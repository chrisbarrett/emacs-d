...
- [x] 1.1 In `lisp/gfm/gfm-present.el`, add helper
...
- [x] 1.2 Remove `gfm-present--ext-to-lang` and
  `gfm-present--language-from-extension`.

## 2. Fontify body via temp buffer

- [x] 2.1 Add helper `gfm-present--fontify-source PATH LINES` that:
  - opens a temp buffer
  ...
  - returns `(buffer-substring (point-min) (point-max))` (keeps
    text properties)
- [x] 2.2 Rewrite `gfm-present--source-preview-fence` (rename to
...
- [x] 2.3 Update the single call site in
  `gfm-present--render-link-previews` to use the renamed helper.

## 3. Tests

- [x] 3.1 In `lisp/gfm/gfm-present-tests.el`, retire any test
  asserting language-tag in the fence string.
- [x] 3.2 Add tests covering every scenario in the delta spec:
...
- [x] 4.1 Run `make test`.
- [x] 4.2 In `/private/tmp/presentations/2026-05-22T06-33-tg-native-drift-filters.md`,
  ...
  `\`\`\`` characters.
- [x] 4.3 Repeat the check for a link pointing at an `.el` file —
  confirm Lisp fontification.
- [x] 4.4 Run `openspec validate fontify-source-preview-body
  --strict`.
