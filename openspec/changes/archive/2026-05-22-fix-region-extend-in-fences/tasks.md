...
- [x] 1.1 Add buffer-local variable `gfm-pretty-fences--last-selection-bounds`
      (initial value nil) to `lisp/gfm/gfm-pretty-fences.el`.
- [x] 1.2 Add `gfm-pretty-fences--selection-bounds` returning `(BEG . END)`
...
- [x] 1.3 Add `gfm-pretty-fences--range-selected-p (beg end)` that
...
- [x] 2.1 In `gfm-pretty-fences--apply-bordered-display`
      (`lisp/gfm/gfm-pretty-fences.el`), compute the masked
      `after-string` exactly as today and bind it to `after-masked`.
- [x] 2.2 Compute the bare variant `after-bare` as the empty string
      `""`.
- [x] 2.3 Stash both on the body overlay via
      `'gfm-pretty-fences-after-masked` and
      `'gfm-pretty-fences-after-bare` properties.
- [x] 2.4 Set the initial `'after-string` based on
      `gfm-pretty-fences--range-selected-p lbeg lend`.
- [x] 2.5 Confirm the `body-bg-inset` masking overlay path is
      unchanged.

## 3. Body overlay creation — indent path

- [x] 3.1 Apply the same change shape inside
...
- [x] 3.2 Verify visually that mid-block lines (non-last) use a
      plain empty bare variant.

## 4. Selection-aware after-string swap

- [x] 4.1 Add `gfm-pretty-fences--update-selection` to walk every
...
- [x] 4.2 For each matching overlay, read both stashed variants and
      set `'after-string` to bare iff the overlay's range overlaps
      current selection bounds; masked otherwise.
- [x] 4.3 Memoise: early-return when current bounds `equal` last
      stored bounds; update `gfm-pretty-fences--last-selection-bounds`
      after the walk.

## 5. Hook wiring

- [x] 5.1 Add `gfm-pretty-fences--on-enable` and
...
- [x] 5.2 Wire the on-enable / on-disable callbacks into the fences
      decorator registration so they fire alongside the existing
      lifecycle (mirror
      `gfm-pretty-tables.el:1215-1232`).
- [x] 5.3 Clear `gfm-pretty-fences--last-selection-bounds` on
      disable.
...
- [x] 6.2 Add `lang-markdown/gfm-pretty-fences-body-after-string-bare-on-selection`:
...
- [x] 6.3 Add `lang-markdown/gfm-pretty-fences-body-after-string-restored-after-deselect`:
      same setup, then deactivate the mark, run update, assert
      `after-string` equals the stashed masked variant.
- [x] 6.4 Add `lang-markdown/gfm-pretty-fences-body-overlay-stashes-both-variants`:
      assert every fenced body overlay carries non-nil
      `gfm-pretty-fences-after-masked` and
      `gfm-pretty-fences-after-bare` overlay properties.
- [x] 6.5 Add `lang-markdown/gfm-pretty-fences-rebuild-honours-active-selection`:
...
- [x] 6.6 Add `lang-markdown/gfm-pretty-fences-selection-bounds-honours-evil-visual`:
...
- [x] 7.1 Run `make test-quick` and confirm green.
- [x] 7.2 Run `make test` (full suite incl. narrowing-regression) and
      confirm green.
- [x] 7.3 Open `/Users/chris/src/chrisbarrett/may-i/REFERENCE.md` in a
...
- [x] 7.4 Repeat 7.3 with vanilla `set-mark` + line-stretch selection
