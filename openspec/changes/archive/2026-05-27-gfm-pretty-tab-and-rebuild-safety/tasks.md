...
- [x] 1.1 In `lisp/gfm/gfm-pretty-engine.el`, change the `(null
      ...
      of returning `nil`.
- [x] 1.2 Add an ERT test in `lisp/gfm/gfm-pretty-tests.el` reproducing
...
- [x] 2.1 In `lisp/gfm/gfm-pretty.el`, add `defvar-keymap
      gfm-pretty-mode-map` and wire it into the `define-minor-mode
      gfm-pretty-mode` declaration via `:keymap`.
- [x] 2.2 Implement `gfm-pretty-tab-dwim` per the dispatch table in
      ...
      no-op.
- [x] 2.3 Bind `TAB` to `gfm-pretty-tab-dwim` in `gfm-pretty-mode-map`.
- [x] 2.4 Add ERT tests in `lisp/gfm/gfm-pretty-tests.el` for each
...
- [x] 3.1 Run `make test-quick` and confirm new tests pass and no
      existing test regresses.
- [x] 3.2 Manual smoke test in `gfm-mode`: press `TAB` on a callout
