...
- [x] 1.1 Add optional `(buffer beg end)` parameters to
      `gfm-tables--visible-width--compute`; default to nil for
      backwards compatibility
- [x] 1.2 Implement the overlay-walk branch: when source-buffer
...
- [x] 1.3 Update `gfm-tables--visible-width` (the memoised wrapper)
      to thread the new args through
- [x] 1.4 Restructure `gfm-tables--fontify-cell` to measure cell
      width in the source buffer before copying the cell text to
      the scratch fontify buffer
- [x] 1.5 Add a focused test in `tests.el` for the overlay path:
...
- [x] 8.1 Implement `gfm-links--reveal` mirroring
...
- [x] 14.2 Reference resolution tests: alist build, first-def-
       wins, broken-reference no-decoration, alist invalidation
       on definition edit
- [x] 14.3 Suppression test: confirm no `composition` text
       property under our mode; confirm composition still
       applied in buffers where the mode is off
- [x] 14.4 Reveal tests: per-window reveal independence; whole-
       link reveal when point on either overlay
- [x] 14.5 RET test: follows URL on link; falls through off link
- [x] 14.6 xref backend test: jumps to definition line; defers
       off reference links
- [x] 14.7 Eldoc test: returns URL on link; nil off link
- [x] 14.8 Width-walker overlay-path test (from task 1.5);
       table-cell-with-decorated-link test confirming column
       width matches visible width including icon
- [x] 14.9 Narrowing-regression test under
...
- [x] 15.1 Run `make test-quick`
- [x] 15.2 Run `make test` (full suite)
- [x] 15.3 Run `nix develop --command bash -c 'make test'` to
       confirm the prek hook passes in the devShell (per
       CLAUDE.md devShell requirement for tree-sitter grammars)
- [x] 15.4 Manual smoke: open a markdown file with a mix of
...
- [x] 15.5 Run `openspec validate add-gfm-link-overlays` and
       confirm green
