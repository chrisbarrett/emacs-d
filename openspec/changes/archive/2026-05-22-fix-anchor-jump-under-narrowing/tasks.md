...
- [x] 1.1 In `lisp/gfm/gfm-pretty-links.el`, declare public defvar
  `gfm-pretty-links-after-anchor-jump-functions` (abnormal hook, one
  argument: target buffer position).
- [x] 1.2 Modify `gfm-pretty-links--jump-to-anchor` so the heading
  ...
  the hook.
- [x] 1.3 Add ERT tests in `lisp/gfm/gfm-pretty-links-tests.el`:
...
- [x] 2.1 In `lisp/gfm/gfm-present.el`, define
...
- [x] 2.2 In `gfm-present-mode`'s enable branch, add
  `gfm-present--after-anchor-jump` to
  `gfm-pretty-links-after-anchor-jump-functions` with LOCAL=t.
- [x] 2.3 In `gfm-present-mode`'s disable branch, remove the same
  function from the buffer-local hook.
- [x] 2.4 Add `(declare-function gfm-pretty-links--jump-to-anchor ...)`
...
- [x] 2.5 Add ERT tests in `lisp/gfm/gfm-present-tests.el`:
...
- [x] 3.2 Live-verify in `/private/tmp/presentations/2026-05-22T06-33-tg-native-drift-filters.md`:
...
- [x] 3.3 Sanity-check non-present-mode paths: in a plain markdown
...
- [x] 3.4 Run `openspec validate fix-anchor-jump-under-narrowing
