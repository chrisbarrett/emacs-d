...
- [x] 1.1 Create `lisp/dune-mode/` and `lisp/opam-mode/` directories.
- [x] 1.2 Create empty `lisp/dune-mode/dune-mode.el` with header
      (`;;; dune-mode.el --- Major mode for Dune build config files -*- lexical-binding: t; -*-`),
      Commentary, `(provide 'dune-mode)`, and footer.
- [x] 1.3 Create empty `lisp/dune-mode/dune-mode-tests.el` with
      header and `(require 'ert)` / `(require 'dune-mode)`.
- [x] 1.4 Create empty `lisp/opam-mode/opam-mode.el` with header
      (`;;; opam-mode.el --- Major mode for OPAM package config files -*- lexical-binding: t; -*-`),
      Commentary, `(provide 'opam-mode)`, and footer.
- [x] 1.5 Create empty `lisp/opam-mode/opam-mode-tests.el` with
      header and `(require 'ert)` / `(require 'opam-mode)`.
- [x] 1.6 Confirm `lisp/dune-mode/` and `lisp/opam-mode/` are picked
      up by the autoloads scan.

## 2. `dune-mode` extraction (red → green)

- [x] 2.1 Write failing test `dune-mode/feature-provided` in
      `dune-mode-tests.el`.
- [x] 2.2 Write failing test `dune-mode/parent-is-lisp-data-mode`.
- [x] 2.3 Write failing test
...
- [x] 2.4 Write failing test `dune-mode/dune-txt-does-not-match`.
- [x] 2.5 Write failing test `dune-mode/comment-add-is-zero` that
      activates the mode in a temp buffer and asserts
      `(local-variable-p 'comment-add)` and `comment-add` is 0.
- [x] 2.6 In `lisp/dune-mode/dune-mode.el`, define `dune-mode` via
      `define-derived-mode dune-mode lisp-data-mode "Dune Config"`
      with `(setq-local comment-add 0)` in the body and a
      `;;;###autoload` cookie.
- [x] 2.7 Add `;;;###autoload` `add-to-list 'auto-mode-alist` for
      `(rx "/dune" (? "-" (or "workspace" "project")) eos)`
      pointing at `#'dune-mode`.
- [x] 2.8 Verify all dune tests pass.

## 3. `opam-mode` extraction (red → green)

- [x] 3.1 Write failing test `opam-mode/feature-provided` in
      `opam-mode-tests.el`.
- [x] 3.2 Write failing test `opam-mode/parent-is-conf-colon-mode`.
...
- [x] 3.4 Write failing test `opam-mode/hook-variable-exists`
      asserting `(boundp 'opam-mode-hook)` after load.
- [x] 3.5 In `lisp/opam-mode/opam-mode.el`, define `opam-mode` via
      `define-derived-mode opam-mode conf-colon-mode "OPAM Config"`
      with a `;;;###autoload` cookie.
- [x] 3.6 Add `;;;###autoload` `add-to-list 'auto-mode-alist` for
      `(rx ".opam" eos)` pointing at `'opam-mode`.
- [x] 3.7 Verify all opam tests pass.

## 4. Delete legacy definitions

- [x] 4.1 Delete the `dune-config-mode` `define-derived-mode` block
      and its `auto-mode-alist` push from
      `modules/lang-ocaml/lib.el`.
- [x] 4.2 Delete the `opam-config-mode` `define-derived-mode` block
      and its `auto-mode-alist` push from
...
- [x] 4.3 Move the dune mode-tests
...
- [x] 7.1 During archive, the `## MODIFIED Requirements` block in
...
- [x] 7.2 This change's spec-conventions delta assumes
...
- [x] 8.1 Audit `modules/lang-ocaml/lib.el` for any stray
      references to the renamed mode symbols.
- [x] 8.2 Audit `modules/lang-ocaml/tests.el` for stray references
      to the moved tests.
- [x] 8.3 Run `make test` and confirm green across the full suite.
- [x] 8.4 Run `nix develop --command bash -c 'pre-commit run --all-files'`
