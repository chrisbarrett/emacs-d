...
- [ ] 1.1 In `lisp/gfm/gfm-pretty-links.el`, define two `rx`
  constants: `gfm-pretty-links--source-range-url-rx` matching
  `<path>#L<digits>[-L<digits>]` and
  `gfm-pretty-links--diff-url-rx` matching
  `diff:<base>...<head>[#<path>]`.
- [ ] 1.2 Define `gfm-pretty-links--skip-url-p URL` returning non-nil
  when URL matches either regex.
- [ ] 1.3 Extend the filter pass in
  `gfm-pretty-links--filter-overlapping` to drop records whose
...
- [ ] 2.1 Add ERT tests in `lisp/gfm/gfm-pretty-links-tests.el`
...
- [ ] 3.3 Confirm RET on a source-range link now opens the file
  narrowed to the line range with the focus overlay
  (gfm-present-follow-link path takes over).
- [ ] 3.4 Run `openspec validate skip-source-links-in-pretty
