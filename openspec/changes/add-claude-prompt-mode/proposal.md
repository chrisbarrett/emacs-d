...
- New `claude-prompt-mode`: a minor mode layered on the prompt file's `gfm-mode`, activated via `server-visit-hook` (server-visited buffers only) when the editor wrapper has registered a context for the file, or as a fallback when the buffer's filename matches the Claude prompt-file pattern. No global `find-file-hook`.
...
- New `lisp/claude-prompt/` package (`claude-prompt.el` + `claude-prompt-tests.el`): regexp, context register, server-visit setup, ring, consult sources, repo classification — a proper package with `provide` and autoload cookies.
- `modules/claude/init.el`: activation wiring (adds `claude-prompt-setup-check-buffer` to `server-visit-hook`). No `packages.eld` change — `with-editor` (Magit) and `consult` resolve via existing modules.
