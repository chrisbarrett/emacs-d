...
- [ ] 1.1 Add failing tests for `pane_layout` validation: accepts
      `"tall"` / `"wide"`; rejects other strings, non-strings, and
      empty strings; absent field is permitted on every kind
- [ ] 1.2 Extend `+presentation--validate-slide` to enforce
      `:pane-layout` rules; tests green
- [ ] 1.3 Add coercion test confirming snake_case → kebab-case
      `pane_layout` → `:pane-layout` on the MCP boundary

## 2. Tmux geometry effect planner

- [ ] 2.1 Add failing tests for `+presentation--pane-layout-effects`:
...
- [ ] 2.2 Implement `+presentation--pane-layout-effects`; tests green
- [ ] 2.3 Add failing test that the planner emits no effects when the
      requested layout matches the session's `:pane-layout` slot

## 3. Render integration & idempotence

- [ ] 3.1 Add failing test that `+presentation--render-slide` runs
