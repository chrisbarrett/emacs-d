...
- [ ] 1.1 Write failing tests in `lisp/gfm/gfm-pretty-tests.el` for a
...
- [ ] 1.2 Implement `gfm-pretty-links--classify-url` using `rx`
- [ ] 1.3 Extend the `gfm-pretty-links--link` cl-struct with a
...
- [ ] 6.1 Write failing tests for
      ...
      `default-directory`
- [ ] 6.2 Implement class-dispatched RET handler in
...
- [ ] 7.1 Write failing tests for the eldoc handler asserting:
...
- [ ] 7.2 Rewrite `gfm-pretty-links--eldoc-function` to build the
...
- [ ] 8.1 `make test-quick` passes
- [ ] 8.2 `make test` (full suite) passes
- [ ] 8.3 Manual check in a real markdown buffer: web, anchor, and
...
- [ ] 8.4 Confirm narrowing-regression tag still passes (`make
