...
- [ ] 2.4 Failing test: tool returns an alist with `key`, `path`,
      and `slide_count` matching `1 + (length file_slides)`
- [ ] 2.5 Failing test: invalid `slug` (empty, contains `/`, or
      contains whitespace) signals a `user-error` without writing
      any file
- [ ] 2.6 Failing test: any entry in `file_slides` whose `kind`
      is not `"file"` signals a `user-error`; no file is written;
      no session is created
- [ ] 2.7 Implement: register `present_document` via
      `claude-code-ide-make-tool`; argument coercion mirrors
      `start_presentation`
- [ ] 2.8 Update `get_deck`-style title behaviour: narrative slide
...
- [ ] 3.1 Failing test for deck-search helper: an exact path-and-
      range match returns the slide index; non-match returns nil
- [ ] 3.2 Failing test: `slide:N` URL parses to integer N; invalid
      forms (`slide:foo`, `slide:`) parse to nil
- [ ] 3.3 Failing test: `path#L42-L67` URL parses to
      `(path . (42 . 67))`; `path#L42` parses to
      `(path . (42 . 42))`; missing anchor parses to nil
- [ ] 3.4 Failing test: dispatch fn receives URL + session key;
      returns symbol describing dispatch (`'goto-slide` /
      `'find-file-fallback` / `'pass-through`) without performing
      I/O
- [ ] 3.5 Failing test: `slide:2` against a session with deck
      length 3 dispatches to `'goto-slide` with index 2
- [ ] 3.6 Failing test: `path.el#L42-L67` against a deck containing
...
- [ ] 3.7 Failing test: `https://...` and plain `path.el` (no
      anchor) dispatch to `'pass-through`
- [ ] 3.8 Failing test: dispatch is gated by buffer-local
      `+presentation--session-key` AND major mode `markdown-mode`
      — narrative buffers in unrelated markdown contexts pass
      through
- [ ] 3.9 Implement: bind `RET` (and optionally `mouse-1`) in
...
- [ ] 4.1 Update `modules/presentation/spec.md`: extend Narrative
...
- [ ] 4.2 Update tool descriptions in `init.el` to recommend
      doc-first usage and reference `present_document`
- [ ] 4.3 Rewrite `~/.claude/rules/...presentation.md` (Nix
...
- [ ] 4.4 Update `CLAUDE.md` if it references the slide-deck
...
- [ ] 5.3 Confirm `.claude/presentations/` is gitignored in a
      fresh checkout
