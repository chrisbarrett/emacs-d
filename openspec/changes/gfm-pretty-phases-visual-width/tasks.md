...
- [ ] 1.1 Add `:phase` slot to the decorator struct in
...
- [ ] 1.2 Add `gfm-pretty--decorators-by-phase` helper that returns the
      decorator list phase-ordered (`atoms → containers → overlays`)
      with intra-phase order unspecified
- [ ] 1.3 Route every `dolist` over `gfm-pretty--decorators` /
...
- [ ] 1.4 Implement `gfm-pretty--visual-line-width LBEG LEND` walking
...
- [ ] 2.1 Declare `:phase 'atoms` on the `gfm-pretty-define-decorator
      'links` form in `lisp/gfm/gfm-pretty-links.el`
- [ ] 2.2 Tag every `display`-bearing overlay / text-property
...
- [ ] 3.5 Drop the wrap-simulation call in the non-overflow branch
      (visual width ≤ content budget); rely on
      `gfm-pretty-callouts--right-after` for fixed `align-to`
      placement
- [ ] 3.6 Audit `gfm-pretty-callouts--callout-top-strings` /
...
- [ ] 4.1 Declare `:phase 'containers` on the
      `gfm-pretty-define-decorator 'fences` form in
      `lisp/gfm/gfm-pretty-fences.el`
- [ ] 4.2 Audit fences for `gfm-pretty--max-line-width` /
...
- [ ] 4.3 Where the fence body uses `gfm-pretty--simulate-wrap`, feed
      it `gfm-pretty-visualised-string` of the body range

## 5. Containers phase: `tables`

- [ ] 5.1 Declare `:phase 'containers` on the
      `gfm-pretty-define-decorator 'tables` form in
      `lisp/gfm/gfm-pretty-tables.el`
- [ ] 5.2 Audit tables' existing visual-width walker (currently
...
- [ ] 6.1 Declare `:phase 'overlays` on
      `gfm-pretty-define-decorator 'blockquotes`
- [ ] 6.2 Declare `:phase 'overlays` on
      `gfm-pretty-define-decorator 'hrule`
- [ ] 6.3 Declare `:phase 'overlays` on
      `gfm-pretty-define-decorator 'link-previews`

## 7. Delete the raw-char width helper

- [ ] 7.1 `git grep gfm-pretty--max-line-width` and confirm no
      remaining callers
- [ ] 7.2 Remove `gfm-pretty--max-line-width` from
      `lisp/gfm/gfm-pretty-engine.el`
- [ ] 7.3 Byte-compile the gfm-pretty modules; resolve any caller
      that surfaces

## 8. Tests

- [ ] 8.1 Add a test to the callouts test suite that opens a buffer
...
- [ ] 8.6 Add a window-resize test: a callout-with-link buffer
