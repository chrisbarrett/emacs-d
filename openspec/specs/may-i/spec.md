...
Major mode (`may-i-config-mode`) derived from `lisp-data-mode` for
`may-i` config files. The library at `lisp/may-i/may-i.el` owns
activation on may-i config paths, faces for decision verbs and reason
strings, font-lock for def heads / top-level forms / parser
attributes / style names / fact keywords, apheleia formatter wiring,
may-i-scoped indent rules, and an imenu composing Rules, Parsers,
Arg styles, Definitions, Checks, Loads, and Safe env vars.
