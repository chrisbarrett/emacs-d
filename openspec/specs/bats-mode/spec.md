# bats-mode Specification

## Purpose

Behaviour-facing. Defines `bats-mode`, a major mode derived from
`bash-ts-mode` for editing [Bats](https://bats-core.readthedocs.io/)
`.bats` test files. Covers mode activation, bats-specific directive
font-lock, profile-gated helper-library assertion font-lock,
`$BATS_*` special-variable highlighting, and an imenu that composes
Tests, Fixtures, and inherited bash functions while preserving the
underlying `bash-ts-mode` editing experience.
