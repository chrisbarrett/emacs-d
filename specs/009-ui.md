...
Alert clears automatically after dwelling on tab for
`+tab-bar-alert-clear-delay` seconds.
...
`hl-line` with `hl-line-sticky-flag` disabled (only highlight in focused
window).
...
`goto-address-mode` enabled in prog/text/conf/magit-process modes. Disabled in
org-mode and org-agenda-mode (handled natively).
...
`switch-to-buffer-in-dedicated-window`: `pop` `window-combination-resize`: `t`
...
| Function                        | Purpose                         |
| :------------------------------ | :------------------------------ |
| `+tty-frame-use-box-characters` | Configure display table for TTY |
