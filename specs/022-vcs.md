...
| File                             | Purpose                                            |
| :------------------------------- | :------------------------------------------------- |
| `init/init-vcs.el`               | Magit, transient, git-timemachine, forge           |
...
When entering `git-commit-mode` with empty message, automatically enter evil
insert state.
...
`browse-at-remote-get-url` advised to use revision from active git-timemachine
session.
...
Tab-per-worktree workflow inspired by zellij. Each tab represents a git
worktree.
...
Files opened via claude-code-ide or emacsclient route to correct frame via
`+worktrees-find-file`.

#### Forge Checkout Integration

`forge-checkout-worktree` configured via
`forge-checkout-worktree-read-directory-function` to create worktrees
in `.worktrees/{number}-{head-ref}`.
...
| `+worktrees-destroy-current`        | Delete worktree and branch            |
...
| :------------------------------------ | :------------------------------------- |
| `+worktrees-current-worktree-path`    | Get worktree path for current directory |
| `+worktrees-in-child-worktree-p`      | True if in a child worktree             |
